...
of coverage against normalized coverage.

```{r gbc-plot-depth-by-breadth-normalized}
ggplot(data=as.data.frame(gbc$pol), aes(x=coverage, y=depthOfCoverage / breadthOfCoverage, color=factor(n.subjects))) + geom_point(size=1.5) + scale_color_viridis_d(alpha=.8) + xlim(0, 1) + ylim(0, 5)
```
...
ggplot(data=subset(as.data.frame(gbc$pol), n.subjects>1), aes(x=factor(n.subjects), y=depthOfCoverage / breadthOfCoverage)) + geom_jitter(size=1.5)
```

Finally, a similar picture is obtained via a histogram plot.

```{r gbc-plot-depth-by-breadth-by-contigs-hist}
ggplot(data=subset(as.data.frame(gbc$pol), n.subjects>1), aes(depthOfCoverage/breadthOfCoverage)) + geom_histogram() + facet_wrap(. ~ n.subjects, scale="free")
```
